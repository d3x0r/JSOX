	...
	return {
		...
		_write(msg,complete_at_end) {
			...
			function recoverIdent(cInt) {
				console.trace( "Recover Ident char:", cInt, String.fromCodePoint(cInt), "word:", word );
				...
				while( !completed && status && ( n < buf.length ) )
				{
					...
					case 58/*':'*/:
						...
						} else if( parse_context == CONTEXT_OBJECT_FIELD || parse_context == CONTEXT_CLASS_FIELD  )
						{
							...
							console.log( "Override colon found, allow class redefinition" );
							redefineClass = true;
							break;
						} else {
					...
					case 125/*'}'*/:
						//if(_DEBUG_PARSING)
						console.log( "close bracket context:", word, parse_context, val.value_type, val.string );
...
JSOX.fromJSOX = function( prototypeName, o, f ) {
	...
	fromProtoTypes.set( prototypeName, {protoCon: o.constructor, cb:f } );

	...
	return {
		...
		_write(msg,complete_at_end) {
				...
				case VALUE_OBJECT:
					if( val.className ) { 
						...
						if( !fp )
							fp = fromProtoTypes.get( val.className );
						val.className = null;
						if( fp ) return val.contains = fp.call( val.contains ); 
					}
					return val.contains;
				case VALUE_ARRAY:
					_DEBUG_PARSING_DETAILS && console.log( "Array conversion:", arrayType );
					if( arrayType >= 0 ) {
			...
			function objectPush() {
				...
				elements[val.name] = convertValue();
				_DEBUG_PARSING_DETAILS && console.log( "Updateed value:", current_class_field, val.name, elements[val.name] );
			...
			function openObject() {
				...
				// common code to push into next context
				{
					...
					if( _DEBUG_PARSING ) console.log( "Begin a new object; previously pushed into elements; but wait until trailing comma or close previously:%d", val.value_type );
						...
						if( parse_context == CONTEXT_CLASS_FIELD ) {
							...
							if( current_class ) {
								...
								if( arrayType != -3 && !val.name ) {
									//if( val.value_type !== VALUE_ARRAY && val.value_type !== VALUE_OBJECT ) 
									{
...
/* init prototypes */
{
	...
	fromProtoTypes.set( "map", function (){
		var newMap = new Map();
		for( key in this ) {
			_DEBUG_PARSING_DETAILS && console.log( "Copy key:", key, this[key]);
			newMap.set( key, this[key] );
		}
		_DEBUG_PARSING_DETAILS && console.trace( "------------------ NEW MAP -------------- ", newMap );
		return newMap;
	} );

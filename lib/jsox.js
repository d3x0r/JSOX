...
const contexts = [];
function getContext() { var ctx = contexts.pop(); if( !ctx ) ctx = { context : CONTEXT_UNKNOWN, current_class : null, arrayType : -1, elements : null, element_array : null }; return ctx; }
	...
	return {
		...
		_write(msg,complete_at_end) {
			...
			function arrayPush() {
				...
				case VALUE_NUMBER:
					if( isBigInt ) { element_array.push( BigInt(string) ); return; }
					if( date_format ) { element_array.push( new Date( string ) ); return; }
				...
				case VALUE_ARRAY:
					if( arrayType >= 0 ) {
						var ab = new ArrayBuffer( val.contains.length );
						var u8 = new Uint8Array(ab);
						//ab.transfer( val.contains );
						for( var xx = 0; xx < val.contains.length; xx++ ) { u8[xx] = val.contains[xx] } 
			...
			function objectPush() {
				switch( val.value_type ){
				case VALUE_NUMBER:
					if( isBigInt ) { elements[val.name] = BigInt(string); return; }
					if( date_format ) { elements[val.name] = new Date( string ); return; }

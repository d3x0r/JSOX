	...
	return {
		...
		_write(msg,complete_at_end) {
							...
							if( !nextObj ) {
								{
									...
									while( ctx && p < val.contains.length ) {
										...
										_DEBUG_REFERENCES && console.log( "Doing next context??", p, val.contains.length );
										ctx = ctx.next;
										p++;
									}
			...
			function openObject() {
				...
				if( _DEBUG_PARSING ) console.log( "Begin a new object; previously pushed into elements; but wait until trailing comma or close previously ", val.value_type, val.className );
			...
			function openArray() {
				...
				{
					let old_context = getContext();
					if( _DEBUG_PARSING ) console.log( "Begin a new array; previously pushed into elements; but wait until trailing comma or close previously ", val.value_type );

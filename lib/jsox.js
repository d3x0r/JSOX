			...
			function openObject() {
				...
				if( parse_context == CONTEXT_UNKNOWN ) {
					if( word == WORD_POS_FIELD ) {
						...
						if( !cls ) {
							classes.push( cls = { name : val.string, protoObject:{}, fields : [] } );
				...
				} else if( parse_context === CONTEXT_IN_ARRAY || parse_context === CONTEXT_OBJECT_FIELD_VALUE ) {
				...
				while( !completed && status && ( n < buf.length ) )
				{
					...
					case 125/*'}'*/:
						...
						if( parse_context == CONTEXT_CLASS_FIELD ) {
							...
							if( _DEBUG_PARSING )
								console.log( "close object; empty object", val, elements );
							val.value_type = VALUE_OBJECT;
					...
					case 44/*','*/:
						...
						if( parse_context == CONTEXT_CLASS_FIELD ) {
							...
							if( current_class ) {
								...
								if( val.value_type != VALUE_UNSET ) {
									console.log( "pushing object field:", val );
									objectPush();
									RESET_VAL();
								}
							} else {

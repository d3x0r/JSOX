			...
			while( status && ( input = inQueue.shift() ) ) {
				...
				while( !completed && status && ( n < buf.length ) ) {
					...
					case 91/*'['*/:
						...
						// the bracket is a class tag. A number is never a class name, so a
						// container opening right after one is a run-on: `[8 {}]` parsed as
						// [{}], `[8 []]` as [[]].
						if( parse_context !== CONTEXT_UNKNOWN && val.value_type === VALUE_NUMBER ) {
							status = false;

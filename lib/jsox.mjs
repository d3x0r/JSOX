...
//
const _DEBUG_LL = true;
const _DEBUG_PARSING = true;
	...
	return {
		...
		_write(msg,complete_at_end) {
					...
					if( arrayType >= 0 ) {
						...
						for( lvl = 0; lvl < val.contains.length; lvl++ ) {
							...
							if( _DEBUG_PARSING_DETAILS ) {
								console.log( "Looking up idx:", idx, "in", obj );
							}
							let nextObj = obj[val.contains[lvl]];

							if( _DEBUG_PARSING_DETAILS ) {
								console.log( "Resolve path:", lvl, idx,"in", obj, context_stack.length, val.contains.toString() );
							}
							if( !nextObj ) {
								{
									let ctx = context_stack.first;
									let p = 0;
									while( ctx && p < val.contains.length ) {
										...
										if( ctx.next ) {
											if( "number" === typeof thisKey ) {
												...
												const asdf = ctx.next.node.element_array;
												const actualObject = ctx.next.node.elements;

												_DEBUG_REFERENCES && console.log( "Number in index... tracing stack...", obj, actualObject, ctx.next?.next?.node );

												if( asdf && thisKey >= asdf.length ) {
													_DEBUG_REFERENCES && console.log( "AT ", p, val.contains.length );
													if( p === (val.contains.length-1) ) {
														...
														if( ctx.next.next && thisKey === asdf.length ) {
															...
															/*
															console.log( "Using context next building object...", nextObj
																			, "IS THIS BETTER?", ctx.next.node );
						                           */
															ctx = ctx.next;
															p++;
															obj = nextObj;
													...
													console.log( "This is one broken path...", ctx, ctx.node, thisKey );
													throw new Error( "Array overflow in history." );
												}
												//console.log( "Stack element:", asdf, thisKey );
											} else {
											...
											if( _DEBUG_REFERENCES )  {
												const a = ctx.next.node.element_array;
										...
										//console.log( "Doing next context??", p, val.contains.length );
										ctx = ctx.next;
										p++;
									}
								}
			...
			function arrayPush() {
				...
				if( arrayType == -3 )  {
					_DEBUG_PARSING && console.log(" Array type -3?", val.value_type, element_array, "Last reference push?", savedArray );

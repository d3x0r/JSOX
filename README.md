...
### Summary of Changes from JSON6/JSON
  ...
  - Numbers - underscore digit separation in numbers, octal and binary formats; all javascript number notations. ~~(Addtionally support leading `0` to interpret as octal as C, C++ and other languages support.)~~ A leading `0` is just a digit; see [Leading 0 Octal](#leading-0-octal).
...
The parser source is `src/jsox.mjs`; it is the only hand-edited file.  Everything
in `lib/` is generated from it by `npm run build`: `src-to-lib.mjs` stamps the
package version into `lib/jsox.mjs`, `mjs-to-cjs.mjs` derives `lib/jsox.js`, and
rollup produces the minified and gzipped variants.  Edits made directly in `lib/`
are overwritten on the next build.  The generated files are committed so that a
checkout works without a build step and a tagged release matches what was published.
...
The rationale: JSOX is a data format, not source code.  The leading-zero-means-octal convention lives in
...
`Number()` has never accepted.  The only places JSOX departs from `Number()` are deliberate readability
extensions: `_` digit separators, a leading or trailing `.`, the `n` BigInt suffix, and date/time forms.
...
The alternatives were considered and rejected:
  ...
  - **Octal** silently changes the value, disagrees with `Number()`, and nobody hand-writing a
    ...
    (Even file permission bits, the classic octal use case, end up as `3777` rather than `0777` once
    a sticky or setuid bit is involved.)
...
The same stance applies to [JSON6](https://github.com/d3x0r/JSON6), and is stated in [rfc.txt](./rfc.txt) section 6.

## Changelog
- 1.2.130 (inprogress)
    - Document the rationale for leading-zero numbers being decimal; correct the RFC grammar, which still described them as octal.
    - Parser source moved to `src/jsox.mjs`; `lib/jsox.mjs` is now generated by `npm run build` with the package version stamped in, so `JSOX.version` can no longer drift from `package.json`.
